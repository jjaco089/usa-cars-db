truncate table brand cascade;
insert into brand values ('Ford', 1903, null);
insert into brand values ('Chevrolet', 1911, null);
insert into model values (default, 'Ford', 'Mustang', 1964, null);
insert into brand values ('Chrysler', 1925, null);
insert into brand values ('Tesla', 2003, null);

-- 8/17/2024
insert into brand values ('Lincoln', 1917, null);
insert into brand values ('Cadillac', 1902, null);
insert into brand values ('Buick', 1902, null);
insert into brand values ('Dodge', 1914, null);
insert into brand values ('Lucid', 2007, null);
insert into brand values ('Rivian', 2009, null);